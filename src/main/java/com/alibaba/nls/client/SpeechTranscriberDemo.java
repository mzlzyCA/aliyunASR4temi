    ...
    private static SpeechTranscriberListener getTranscriberListener() {
        SpeechTranscriberListener listener = new SpeechTranscriberListener() {
            ...
            public void onTranscriptionResultChange(SpeechTranscriberResponse response) {
                logger.info("【中间结果】task_id: " + response.getTaskId() +
            ...
            public void onTranscriberStart(SpeechTranscriberResponse response) {
                ...
                logger.info("【识别开始】task_id: " + response.getTaskId() + ", name: " + response.getName() + ", status: " + response.getStatus());
            }

            @Override
            public void onSentenceBegin(SpeechTranscriberResponse response) {
                logger.info("【句子开始】task_id: " + response.getTaskId() + ", name: " + response.getName() + ", status: " + response.getStatus());
            }

            //识别出一句话.服务端会智能断句,当识别到一句话结束时会返回此消息
            @Override
            public void onSentenceEnd(SpeechTranscriberResponse response) {
                logger.info("【句子结束】task_id: " + response.getTaskId() +
                    ", name: " + response.getName() +
                    //状态码 20000000 表示正常识别
                    ", status: " + response.getStatus() +
                    //句子编号，从1开始递增
            ...
            public void onTranscriptionComplete(SpeechTranscriberResponse response) {
                logger.info("【识别完成】task_id: " + response.getTaskId() + ", name: " + response.getName() + ", status: " + response.getStatus());
            }

            @Override
            public void onFail(SpeechTranscriberResponse response) {
                // TODO 重要提示： task_id很重要，是调用方和服务端通信的唯一ID标识，当遇到问题时，需要提供此task_id以便排查
                logger.info("【识别失败】task_id: " + response.getTaskId() +  ", status: " + response.getStatus() + ", status_text: " + response.getStatusText());
            }
        };
    ...
    public void process(String filepath) {
        ...
        try {
            ...
            while ((len = fis.read(b)) > 0) {
                logger.info("【数据发送】send data pack length: " + len);
            ...
            logger.info("【状态信息】ASR wait for complete");
            transcriber.stop();
            logger.info("【状态信息】ASR latency : " + (System.currentTimeMillis() - now) + " ms");
        } catch (Exception e) {
...
//        String filepath = "src/main/resources/nls-sample-16k.wav";
        String filepath = "src/main/resources/test_16k.wav";
